...
//             dispatch(setUser(response.data.user)); // set user data for register
...
//              dispatch(setUser(response.data.user));               // set user data for login 
...
const authSlice = createSlice({
  name: "auth",
  initialState: {
    data: null, // Store a single user object or null
    status: STATUSES.SUCCESS,
    token: "",
  },
  reducers: {
...
export function registerUser(data) {
  return async function registerUserThunk(dispatch) {
    ...
    try {
      const response = await API.post("/auth/register", data);
      dispatch(setUser(response.data.user)); // Backend returns { msg, user }
      dispatch(setStatus(STATUSES.SUCCESS));
